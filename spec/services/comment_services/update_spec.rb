...
describe "CommentServices::Update" do
  ...
  context "when users edit their comment" do
    let(:service) { CommentServices::Update.new({ post_id: post.id, id: user.id, content: "Sample comment 2" }, user) }
  ...
  context "when users edit other's comment" do
    let(:service) { CommentServices::Update.new({ post_id: post.id, id: user.id, content: "Sample comment 2" }, second_user) }
